...
namespace utilities{ 
	...
	inline Vector3d rotationMatrixError(Matrix3d R_d, Matrix3d R_e)
	{
	...
	//******************************************************************************
	inline Vector3d quaternionError(Matrix3d R_d, Matrix3d R)
	{
		float eta, eta_d;
	...
	inline Vector4d omega2dotq( Matrix3d R, Vector3d omega ) {

		Vector4d dot_q;
		Matrix3d I_3 =  Matrix3d::Identity();
		Vector3d epsilon_e;
		Vector4d qe = utilities::rot2quat( R );
		epsilon_e << qe(1), qe(2), qe(3);

		Matrix3d Se = utilities::skew( epsilon_e );

		dot_q(0) = -0.5*( epsilon_e(0)*omega(0) + epsilon_e(1)*omega(1) + epsilon_e(2)*omega(2) );
		Vector3d dot_eta = 0.5*( qe[0] * I_3  - Se )*omega;
...
//--------------------SCREW THEORY FUNCTION----------------------------
	...
	//--------------------MPC PARAM UPDATE FUNCTION----------------------------
	inline Eigen::Vector2d find_intersect(Eigen::Vector2d com_wuav, Eigen::Vector2d pos_pipe, Eigen::Vector2d w_dx, Eigen::Vector2d w_sx){
		double m1, q1, m2, q2;
		Eigen::Vector2d intersect;
		m1 = (w_dx(1)-w_sx(1))/(w_dx(0)-w_sx(0));
		q1 = (w_dx(0)*w_sx(1)-w_sx(0)*w_dx(1))/(w_dx(0)-w_sx(0));

		m2 = (com_wuav(1)-pos_pipe(1))/(com_wuav(0)-pos_pipe(0));
		q2 = (com_wuav(0)*pos_pipe(1)-pos_pipe(0)*com_wuav(1))/(com_wuav(0)-pos_pipe(0));
		intersect(0) = (q2-q1)/(m1-m2);
		intersect(1) = m1*intersect(0)+q1;
		return intersect;
	}

	inline double point_line_dist(Eigen::Vector2d point, Eigen::Vector2d ex1, Eigen::Vector2d ex2){
		double m,q,dist;

		m = (ex1(1)-ex2(1))/(ex1(0)-ex2(0));
		q = (ex1(0)*ex2(1)-ex2(0)*ex1(1))/(ex1(0)-ex2(0));

		dist = abs(point(1)-(m*point(0)+q))/sqrt(1+pow(m,2));
		return dist;
	}

	inline Eigen::Vector2d model_angles(Eigen::Vector2d intersection, Eigen::Vector2d w_dx, Eigen::Vector2d w_sx, Eigen::Vector2d point){
		double dist, h_dx, h_sx, alfa_dx, alfa_sx;
		Eigen::Vector2d alfa_vec;
		dist = sqrt(pow((intersection(0)-point(0)),2)+pow((intersection(1)-point(1)),2));
		h_dx = utilities::point_line_dist(intersection,w_dx,point);
		h_sx = utilities::point_line_dist(intersection,w_sx,point);
		alfa_dx = asin(h_dx/dist);
		alfa_sx = asin(h_sx/dist);
		alfa_vec <<	alfa_dx, alfa_sx;	
		return alfa_vec;
	}
